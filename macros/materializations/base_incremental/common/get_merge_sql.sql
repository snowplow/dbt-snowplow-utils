...
{% macro default__get_merge_sql(target_tb, source, unique_key, dest_columns, incremental_predicates = none) -%}
    ...
    {%- set optimize = config.meta_get('snowplow_optimize') -%}
    {% if optimize %}

        -- run some queries to dynamically determine the min + max of this 'upsert_date_key' in the new data
        {%- set date_column = config.meta_require('upsert_date_key') -%}
        {%- set disable_upsert_lookback = config.meta_get('disable_upsert_lookback') -%} {# We do this for late arriving data possibly e.g. shifting a session start earlier #}
...
{% macro default__get_delete_insert_merge_sql(target_tb, source, unique_key, dest_columns, incremental_predicates) -%}
    ...
    {% if optimize %}
        -- run some queries to dynamically determine the min + max of this 'upsert_date_key' in the new data
        {%- set date_column = config.meta_require('upsert_date_key') -%}
        {%- set disable_upsert_lookback = config.meta_get('disable_upsert_lookback') -%}
