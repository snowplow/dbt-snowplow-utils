...
OUTPUT=$(eval "dbt run --select test_snowplow_delete_from_manifest --target $DATABASE --vars 'models_to_delete: [\"x\",\"y\"]'") || exit 1;
...
eval "dbt run --select test_snowplow_delete_from_manifest expected_snowplow_delete_from_manifest --target $DATABASE --vars 'models_to_delete: [\"a\",\"b\"]'" || exit 1;

eval "dbt test --select test_snowplow_delete_from_manifest --target $DATABASE" || exit 1;
...
OUTPUT=$(eval "dbt run --select test_snowplow_delete_from_manifest expected_snowplow_delete_from_manifest --target $DATABASE --vars 'models_to_delete: [\"a\",\"b\",\"x\",\"y\"]'") || exit 1;
